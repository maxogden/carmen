...
INSERT INTO data (id, geometry, name, search) SELECT ogc_fid, setsrid(wkb_geometry,4326), name_1 AS name, name_1||','||postal AS search FROM import;
...
UPDATE data SET area = st_area(st_geogfromwkb(geometry)) where within(geometry,geomfromtext('POLYGON((-180 -90, -180 90, 180 90, 180 -90, -180 -90))',4326));
" | psql -U postgres $TMP
